...
@task(name="apply_2d_convolution")
...
def task_process_acoustic(
    ...
    seafloor_threshold: list = [-50, 2.4, 1.0],
    seafloor_offset: float = 0.5,
    seafloor_r0: float = 25,
    seafloor_r1: float = 1000,
    seafloor_wtheta: int = 7,
    seafloor_wphi: int = 7,
    ...
    try:
        ...
        angle_alonghsip_threshold = ds_Sv["angle_alongship"].isel(channel=1).rolling(ping_time=7, range_sample=7).mean().quantile([q]).values[0]
        angle_athwartship_threshold = ds_Sv["angle_athwartship"].isel(channel=1).rolling(ping_time=7, range_sample=7).mean().quantile([q]).values[0]

        seafloor_threshold = [seafloor_threshold[0], angle_alonghsip_threshold, angle_athwartship_threshold]
        seafloor_params = {
    ...
    try:
        lines = er.read_lines_csv("/home/user/src/Bottom Detection/CPS_Bottom/bottom_line.csv")
        selected_channel = _pick_channel_by_frequency(ds_Sv, target_frequency)

        depth_da = ds_Sv["depth"].sel(channel=selected_channel)
        max_depth_per_ping = depth_da.max(dim="range_sample", skipna=True)
        deepest_ping_idx = int(max_depth_per_ping.argmax(dim="ping_time").values)

        depth = depth_da.isel(ping_time=deepest_ping_idx).dropna(dim="range_sample", how="all")
        valid_length = len(depth.values)

        ds_Sv = ds_Sv.isel(range_sample=slice(0, valid_length))
        ...
        ds_single = xr.Dataset({
            "Sv": Sv_plot,
            "frequency_nominal": xr.DataArray([selected_channel], dims=["channel"], coords={"channel": [selected_channel]})
        })

        bottom_mask_da, _ = lines.seafloor_mask(
            ds_single.Sv, 
        ...
        above_mask_da = ~bottom_mask_da.astype(bool)

        Sv_aligned_below = ds_Sv["Sv"].rename({"range_sample": "depth"}).assign_coords(depth=bottom_mask_da.depth)
        Sv_aligned_above = ds_Sv["Sv"].rename({"range_sample": "depth"}).assign_coords(depth=above_mask_da.depth)


        mask_below = (bottom_mask_da == 1)
        mask_above = ~mask_below

        Sv_above = xr.where(mask_above, Sv_aligned_above, np.nan)
        Sv_below = xr.where(mask_below, Sv_aligned_below, np.nan)

        Sv_above = Sv_above.transpose("channel", "ping_time", "depth")
        Sv_below = Sv_below.transpose("channel", "ping_time", "depth")

        Sv_above_renamed = Sv_above.rename({"ping_time": "x", "depth": "y"})
        Sv_below_renamed = Sv_below.rename({"ping_time": "x", "depth": "y"})

        Sv_above_renamed = Sv_above_renamed.compute()
        Sv_below_renamed = Sv_below_renamed.compute()

        convolved_above_renamed = apply_2d_convolution.fn(Sv_above_renamed)
        convolved_below_renamed = apply_2d_convolution.fn(Sv_below_renamed)

        convolved_above = convolved_above_renamed.rename({"x": "ping_time", "y": "depth"})
        convolved_below = convolved_below_renamed.rename({"x": "ping_time", "y": "depth"})

       
        convolved_combined = xr.where(mask_above, convolved_above, convolved_below)

        convolved_combined = convolved_combined.transpose("channel", "ping_time", "depth")

        
        convolved_combined_final = xr.DataArray(
            data=convolved_combined.values,      
            dims=ds_Sv["Sv"].dims,               
            coords=ds_Sv["Sv"].coords,           
            name="Sv_convolved"
        )

        ds_Sv["Sv"] = convolved_combined_final
    except Exception as e:
        logger.warning(f"Bottom line masking skipped: {e}")
    ...
    try:
        ...
        convolved_above = convolved_above_renamed.rename({"x": "ping_time", "y": "depth"})
        convolved_combined = xr.where(mask_above, convolved_above, np.nan)
        convolved_combined = convolved_combined.transpose("channel", "ping_time", "depth")


        convolved_combined_final = xr.DataArray(
            data=convolved_combined.values,     
            dims=ds_Sv["Sv_dilated"].dims,               
            coords=ds_Sv["Sv_dilated"].coords,           
            name="Sv_final"
        )

        ds_Sv["Sv_dilated"] = convolved_combined_final
        ...
        ds_Sv["Sv_masked"] = ds_Sv["Sv"].where(final_mask)
            ...
            logger.warning(f"Bottom line masking post filter skipped: {e}")
